...
module.exports = class Task {
  ...
  initialize(done, additionalSteps = []) {
    async.series(
      additionalSteps.concat(this.setPostProcessingOptsSteps(), [
        // Read images info
        (cb) => {
        ...
        (cb) => {
          fs.readdir(this.getGcpFolderPath(), (err, files) => {
            if (err) cb(err);
              ...
              logger.debug(
                `Found ${this.gcpFiles.length} GCP files (${this.gcpFiles.join(
                  " "
                )}) for ${this.uuid}`
              );
              logger.debug(
                `Found ${this.geoFiles.length} GEO files (${this.geoFiles.join(
                  " "
                )}) for ${this.uuid}`
              );
              logger.debug(
                ...
                } image groups files (${this.imageGroupsFiles.join(" ")}) for ${
                  this.uuid
                }`
              );
              logger.debug(
                `Found ${
                  this.alignFiles.length
                } alignment files (${this.alignFiles.join(" ")}) for ${
                  this.uuid
                }`
              );
      ...
      (err) => {
        ...
        if (this.getStatus() === statusCodes.RUNNING) {
          if (err)
            this.setStatus(statusCodes.FAILED, { errorMessage: err.message });
  ...
  setPostProcessingOptsSteps() {
    ...
    return [autoSet("pc-ept"), autoSet("cog"), autoSet("gltf")];
  }
  ...
  start(done) {
      ...
      const saveTaskOutput = (destination) => {
        return (done) => {
          fs.writeFile(destination, this.output.join("\n"), (err) => {
            if (err)
              logger.info(`Cannot write log at ${destination}, skipping...`);
            done();
          });
        };
      };
    ...
    if (this.status.code === statusCodes.QUEUED) {
      ...
      if (this.alignFiles.length > 0) {
        runnerOptions.align = fs.realpathSync(
          path.join(this.getGcpFolderPath(), this.alignFiles[0])
        );
      }
  ...
  restart(options, cb) {
    if (!this.initialized && this.status.code === statusCodes.CANCELED) {
      ...
      [statusCodes.CANCELED, statusCodes.FAILED, statusCodes.COMPLETED].indexOf(
        this.status.code
      ) !== -1
    ) {
